...
- `search-content.js` → `runDeepCrawlSearch`
...
- 浮动按钮 ①～⑦，与 popup 能力对齐
- 监听：`EXTRACT_JD_SEARCH`、`CACHE_JD_SEARCH_URLS`、`RUN_BATCH_DETAIL_FROM_QUEUE`、`DEEP_CRAWL_JD_SEARCH`
- `runDeepCrawlSearch`：每页 `extractJdSearchProducts` → 逐卡 `EXTRACT_URL_IN_NEW_TAB`
...
| `EXTRACT_JD_SEARCH` | search | `{ maxPages }` | `{ ok, data: { keyword, pages, count, products } }` |
...
| type | 载荷 | 响应字段 |
|------|------|----------|
| `APPEND_JSONL_RECORD` | `{ product }` | `{ ok, count, records, filename }` |
| `APPEND_JSONL_RECORDS` | `{ products: [] }` | `{ ok, count, added, records, filename }` |
| `APPEND_PRODUCT_URLS` | `{ urls: [] }` | `{ ok, count, added, urls }` |
| `GET_JSONL_RECORDS` | — | `{ ok, records, count }` |
| `CLEAR_JSONL_RECORDS` | — | `{ ok }` |
| `GET_PRODUCT_URLS` | — | `{ ok, urls, count }` |
| `CLEAR_PRODUCT_URLS` | — | `{ ok }` |
...
| Key | 类型 | 说明 |
|-----|------|------|
| `jd_jsonl_records` | `ProductSource[]` | 全局商品缓存，append 时按 `product_id` \| `sku` \| `url` 去重覆盖 |
| `jd_product_url_queue` | `{ url, sku, title?, search_keyword?, collected_at? }[]` | 批量详情队列 |
...
搜索列表额外：`search_keyword`, `search_page`, `shop_name`；`variants` / `description` 多为 `null`。
